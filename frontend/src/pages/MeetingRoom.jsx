...
export const MeetingRoom = () => {
    ...
    const [videoAvailable, setVideoAvailable] = useState(initialVideoAvailable ?? true)
    const [audioAvailable, setAudioAvailable] = useState(initialAudioAvailable ?? true)
    const [video, setVideo] = useState(getPersistedState('video', initialVideo ?? true))
    const [audio, setAudio] = useState(getPersistedState('audio', initialAudio ?? true))
    ...
    const [cameraFacingMode, setCameraFacingMode] = useState(getPersistedState('cameraFacingMode', initialCameraFacingMode || "user"))
    ...
    }, [username, meetingCode, gotMessageFromServer, addMessage, createBlackSilenceStream, createBlackVideoTrack, createSilentAudioTrack, broadcastMediaState])
    ...
    useEffect(() => {
        if (!localStreamRef.current && !isScreenSharingRef.current && !isGettingUserMediaRef.current) {
    ...
    useEffect(() => {
        if (localStreamRef.current && !isSwitchingCamera && !isSwitchingCameraRef.current) {
            const audioTracks = localStreamRef.current.getAudioTracks()
            audioTracks.forEach((track) => {
                if (track.label && !track.label.includes("MediaStreamAudioDestinationNode")) {
                    const shouldBeEnabled = audioStateRef.current
    ...
    }, [cameraFacingMode, screen, isSwitchingCamera, getUserMedia, videoAvailable])
